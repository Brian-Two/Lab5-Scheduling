...
{ 
   ...
   */
  int *rem_bt = (int *)calloc(n, sizeof(int));
  if (!rem_bt) return;
...
{ 
  ...
  while (complete != n) {
    ...
    for (int j = 0; j < n; j++) {
      if (plist[j].art <= t && rem[j] > 0 && rem[j] < minm) {
        minm = rem[j];
        shortest = j;
        check = 1;
      }
    ...
    if (rem[shortest] == 0) {
      complete++;
      int finish_time = t + 1;
      int wt = finish_time - plist[shortest].bt - plist[shortest].art;
      plist[shortest].wt = wt < 0 ? 0 : wt;
    }
